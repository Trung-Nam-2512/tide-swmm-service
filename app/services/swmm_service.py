...
# Lock để tránh multi-simulation error
...
def write_inp(timeseries: TimeseriesInput) -> str:
    ...
    # Interpolate timeseries (except tide which is already interpolated from API)
    ...
    # timeseries.tide is already interpolated from API, don't interpolate again
...
def run_and_parse_swmm() -> List[Dict[str, Any]]:
    ...
    """
    # Sử dụng lock để tránh multi-simulation error
    with _simulation_lock:
        try:
            ...
            if settings.CACHE_ENABLED:
                ...
                logger.info("Cache disabled - running fresh simulation")
            
            # Chạy mô hình SWMM trực tiếp
            logger.info(f"Running fresh SWMM simulation...")
            ...
            with Simulation(inp_file_path) as sim:
                ...
                # Lưu kết quả vào cache nếu enabled
                if settings.CACHE_ENABLED:
                    with open(cache_file, 'w', encoding='utf-8') as f:
                        json.dump(all_results, f, indent=2)
                    logger.info(f"Results saved to cache: {cache_file}")
                else:
                    logger.info("Cache disabled - results not saved to cache")
